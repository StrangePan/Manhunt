...
public class WorldDataFile extends Properties {
	...
	private Location hunterSpawn = HuntedPlugin.getInstance().getWorld()
			.getSpawnLocation();
	private Location preySpawn = HuntedPlugin.getInstance().getWorld()
			.getSpawnLocation();
	private Location pregameSpawn = HuntedPlugin.getInstance().getWorld()
			.getSpawnLocation();
	...
	public void loadWorldFile() {
		File file = new File(HuntedPlugin.getInstance().getWorld()
				.getWorldFolder().toString()
				+ "/" + location);
		if (!file.exists()) {
			try {
				file.createNewFile();
				...
				HuntedPlugin.getInstance().log(Level.SEVERE,
						"Problem loading the Manhunt World Data file!");
	...
	public void saveWorldFile() {
		...
		try {
			store(new FileOutputStream(file),
					"- Manhunt World Configurations and Settings -");
		} catch (IOException e) {
			HuntedPlugin.getInstance().log(Level.SEVERE,
					"Problem loading the Manhunt World Data file!");
			HuntedPlugin.getInstance().log(Level.SEVERE, e.getMessage());
	...
	public void loadWorldValues() {
		...
		if (containsKey("boxBoundary")) {
			if (getProperty("boxBoundary").length() > 0
					&& getProperty("boxBoundary").equalsIgnoreCase("true")) {
				boxBoundary = true;
			} else {
				boxBoundary = false;
			}
		...
		if (containsKey("noBuild")) {
			if (getProperty("noBuild").length() > 0
					&& getProperty("noBuild").equalsIgnoreCase("true")) {
				noBuild = true;
			} else {
				noBuild = false;
			}
		...
		if (containsKey("mapBoundary")) {
			if (getProperty("mapBoundary").length() > 0) {
				try {
					...
					if (mapBoundary < 64 && mapBoundary > -1)
						mapBoundary = 64;
					if (mapBoundary <= -1)
						mapBoundary = -1;
		...
		if (containsKey("pregameBoundary")) {
			if (getProperty("pregameBoundary").length() > 0) {
				try {
					pregameBoundary = Integer
							.parseInt(getProperty("pregameBoundary"));
					if (pregameBoundary < -1)
						pregameBoundary = -1;
					if (pregameBoundary > 64)
						pregameBoundary = 64;
				} catch (NumberFormatException e) {
					pregameBoundary = 8;
				}
		...
		if (containsKey("noBuildRange")) {
			if (getProperty("noBuildRange").length() > 0) {
				try {
					noBuildRange = Integer
							.parseInt(getProperty("noBuildRange"));
					if (noBuildRange < 0)
						noBuildRange = -1;
				} catch (NumberFormatException e) {
					noBuildRange = 8;
		...
		if (containsKey("hunterSpawn")) {
			if (getProperty("hunterSpawn").split(",").length == 5) {
				...
				try {
					...
					hunterSpawn = HuntedPlugin.getInstance().getWorld()
							.getSpawnLocation();
				}
			} else {
				hunterSpawn = HuntedPlugin.getInstance().getWorld()
		...
		put("hunterSpawn", hunterSpawn.getX() + "," + hunterSpawn.getY() + ","
				+ hunterSpawn.getZ() + "," + hunterSpawn.getPitch() + ","
				+ hunterSpawn.getYaw());
		...
		if (containsKey("preySpawn")) {
			if (getProperty("preySpawn").split(",").length == 5) {
				...
				try {
					...
					preySpawn = HuntedPlugin.getInstance().getWorld()
							.getSpawnLocation();
				}
			} else {
				preySpawn = HuntedPlugin.getInstance().getWorld()
		...
		put("preySpawn", preySpawn.getX() + "," + preySpawn.getY() + ","
				+ preySpawn.getZ() + "," + preySpawn.getPitch() + ","
				+ preySpawn.getYaw());
		...
		if (containsKey("pregameSpawn")) {
			if (getProperty("pregameSpawn").split(",").length == 5) {
				...
				try {
					...
					pregameSpawn = HuntedPlugin.getInstance().getWorld()
							.getSpawnLocation();
				}
			} else {
				pregameSpawn = HuntedPlugin.getInstance().getWorld()
		...
		put("pregameSpawn", pregameSpawn.getX() + "," + pregameSpawn.getY()
				+ "," + pregameSpawn.getZ() + "," + pregameSpawn.getPitch()
				+ "," + pregameSpawn.getYaw());
	}
	...
	public void changeSetting(String setting, Location loc) {
		if (containsKey(setting)) {
			String value = loc.getX() + "," + loc.getY() + "," + loc.getZ()
